...
import { treasurerAdhesionsApi, type AdhesionRow } from '../api/treasurerListApi';
import '../../../components/layout/CrudPage.css';
import './TreasurerDemandesAdhesionPage.css';
...
export function TreasurerDemandesAdhesionPage() {
  ...
  const all = useQuery({
    queryKey: ['treasurer', 'adhesions', 'all'],
    queryFn: () => treasurerAdhesionsApi.list({ page: 1, size: 1000 }),
  });

  const query = useQuery({
    queryKey: ['treasurer', 'adhesions', { page, search, statut }],
    queryFn: () =>
      treasurerAdhesionsApi.list({ page, size: 10, search, filters: { statut } }),
  });

  const valider = useMutation({
    mutationFn: (id: string) => treasurerAdhesionsApi.valider(id),
    onSuccess: (a) => {
      qc.invalidateQueries({ queryKey: ['treasurer', 'adhesions'] });
  ...
  const rejeter = useMutation({
    mutationFn: (id: string) => treasurerAdhesionsApi.rejeter(id),
    onSuccess: (a) => {
      qc.invalidateQueries({ queryKey: ['treasurer', 'adhesions'] });
      setRejecting(null);
