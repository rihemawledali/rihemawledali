...
export function BonsCommandePage() {
  ...
  const stats = {
    ...
    valide: allItems.filter((bon) => bon.statut === 'valide').length,
    restant: allItems.reduce((sum, bon) => sum + (bon.quantiteRestante ?? 0), 0),
    montant: allItems.reduce((sum, bon) => sum + bon.montant, 0),
  };
    ...
    <div className="offers-page">
      ...
      <section className="offers-workspace">
        <div className="crud-toolbar offers-toolbar">
          <FilterBar>
            <SearchInput value={search} onChange={changeSearch} placeholder="Numero, fournisseur, type..." />
            <SelectFilter label="Statut" value={statut} onChange={changeStatus} options={BON_STATUS_OPTIONS} />
          </FilterBar>
        </div>
...
function StockCell({ bon }: { bon: BonCommande }) {
  ...
  return (
    <div className="offer-stock-cell">
      ...
      <progress className="offer-stock-progress" value={assigned} max={total || 1} aria-label="Stock attribue" />
    </div>
  );
}

function OfferMetric(props: any) {
