...
export function TicketsPage() {
  ...
  const assignableBons = (bons.data?.items ?? []).filter((bon) => bon.statut === 'valide' && (bon.quantiteRestante ?? 0) > 0);
  ...
  const selectedBon = (bons.data?.items ?? []).find((bon) => bon.id === effectiveBonId);
  ...
  const stats = {
    ...
    libres: ticketItems.filter((ticket) => ticket.statut === 'en_attente').reduce((sum, ticket) => sum + ticket.quantite, 0),
    attribues: ticketItems.filter((ticket) => ticket.statut === 'attribue').reduce((sum, ticket) => sum + ticket.quantite, 0),
  };
  const groupedRows = groupTicketsByAssignment(ticketItems);
  const pageRows = groupedRows.slice((page - 1) * 10, page * 10);

  const columns: Column<TicketCountRow>[] = [
    {
    ...
    <div className="offers-page">
      ...
      <section className="offers-workspace">
        <div className="crud-toolbar offers-toolbar">
          <FilterBar>
            <SearchInput value={search} onChange={changeSearch} placeholder="Numero, adherent, matricule..." />
            <SelectFilter label="Statut" value={statut} onChange={changeStatus} options={TICKET_STATUS_OPTIONS} />
          </FilterBar>
        </div>
...
function TicketMetric(props: any) {
  return <SimpleMetricCard {...props} className="offer-metric" tonePrefix="offer-metric--" iconClassName="" />;
}
