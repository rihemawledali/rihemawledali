...
const FILTERS: Array<{ key: StatusFilter; label: string; icon: typeof FileText }> = [
  { key: 'all', label: 'Toutes', icon: FileText },
  { key: 'en_attente', label: 'En attente', icon: Clock3 },
  { key: 'validee', label: 'Validées', icon: CheckCircle2 },
  { key: 'refusee', label: 'Refusées', icon: XCircle },
  { key: 'annulee', label: 'Annulées', icon: Ban },
  { key: 'terminee', label: 'Terminées', icon: Archive },
  { key: 'expiree', label: 'Expirées', icon: CalendarX },
];
...
export function AdherentMesDemandesConventionsPage() {
  ...
  const filteredDemandes = useMemo(() => {
    if (filter === 'all') return annotatedDemandes;
    return annotatedDemandes.filter((item) => item.displayStatus === filter);
  }, [annotatedDemandes, filter]);
    ...
    <div className="adh-follow-page">
      ...
      <section className="adh-follow-toolbar">
        <div>
          ...
          <h2>{counts.all} demande{counts.all > 1 ? 's' : ''}</h2>
        </div>
      ...
      <section className="adh-demandes-tabs" aria-label="Filtres demandes">
        {FILTERS.map(({ key, label, icon: Icon }) => (
          <button
            key={key}
            type="button"
            className={`adh-demandes-tab ${filter === key ? 'is-active' : ''}`}
      ...
      {isLoading ? (
        ...
        <section className="adh-empty-card adh-follow-empty">
          ...
          <h3>{filter === 'all' ? 'Aucune demande' : 'Aucune demande dans ce statut'}</h3>
          <p>
            {filter === 'all'
              ? 'Vos prochaines demandes de conventions apparaîtront ici.'
              : 'Changez le filtre pour consulter les autres demandes.'}
          </p>
        ...
        <div className="adh-demandes-list">
          {filteredDemandes.map(({ demande, displayStatus }) => (
            <DemandeCard
              key={demande.id}
              demande={demande}
